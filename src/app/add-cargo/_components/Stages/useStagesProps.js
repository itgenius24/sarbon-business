...
export const useStagesProps = () => {
  ...
  function getStatus(type) {
    switch(type) {
      ...
      case "loadings": {
        if(loadings.address && unloading.address && loadings.location?.value && unloading.location?.value) return "done";
        else if(loadings.address || unloading.address || loadings.location?.value || unloading.location?.value) return "process";
        else return "disabled";
      }
      case "transport": {
