...
export const AccordionMap = ({ gpsHistory, driverPosition, periods}) => {
  const map = useRef(null);
  const mapState = {
    center: driverPosition.length > 0 ? driverPosition : [41.3405737, 69.2928081],
    zoom: 11,
  };

  const shipper = periods.filter(
    (item) => item.type?.[0] === `shipper`
  );

  const consignee = periods.filter(
